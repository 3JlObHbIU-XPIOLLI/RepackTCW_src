...
The files use a ".pak" extension to prevent users from unzipping them accidentally, but
otherwise the are simply normal uncompressed zip files.  A game directory can have multiple
zip files of the form "pak0.pak", "pak1.pak", etc.  Zip files are searched in decending order
...
typedef struct {
	...
	char pakFilename[MAX_OSPATH];               // c:\quake3\baseq3\pak0.pak
...
#ifdef __APPLE__
// Also search the .app bundle for .pak files
		...
		const char *function )
{
	...
	if( Sys_DllExtension( filename )
		...
		|| COM_CompareExtension( filename, ".pak" ) )
	{
	...
	while(search)
	{
		...
		else if(search->pack)
		{
			...
			if(lastSearch && lastSearch->pack)
			{
				...
				// i.e. if VM from pak7.pak fails we won't try one from pak6.pak
...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename)
{
	...
	// strip .pak if needed
	if ( strlen( pack->pakBasename ) > 4 && !Q_stricmp( pack->pakBasename + strlen( pack->pakBasename ) - 4, ".pak" ) ) {
	...
	*listbuf = 0;
	...
	for ( i = 0 ; i < nPotential ; i++ ) {
		...
		if (Q_stricmp(name, com_basegame->string) && Q_stricmpn(name, ".", 1)) {
			// now we need to find some .pak files to validate the mod
			// NOTE TTimo: (actually I'm not sure why .. what if it's a mod under developement with no .pak?)
			// we didn't keep the information when we merged the directory names, as to what OS Path it was found under
			//   so it could be in base path, cd path or home path
			//   we will try each three of them here (yes, it's a bit messy)
			path = FS_BuildOSPath( fs_basepath->string, name, "" );
			nPaks = 0;
			pPaks = Sys_ListFiles(path, ".pak", NULL, &nPaks, qfalse); 
			Sys_FreeFileList( pPaks ); // we only use Sys_ListFiles to check wether .pak files are present
...
void FS_AddGameDirectory( const char *path, const char *dir ) {
	...
	pakfiles = Sys_ListFiles( pakfile, ".pak", NULL, &numfiles, qfalse );

	// sort them so that later alphabetic matches override
	// earlier ones.  This makes pak1.pak override pak0.pak
	...
	for ( i = 0 ; i < fs_numServerReferencedPaks ; i++ )
	{
		...
		if ( !havepak && fs_serverReferencedPakNames[i] && *fs_serverReferencedPakNames[i] ) {
			...
			if (dlstring)
			{
				...
				// Q_strcat cuts off the .pak extension.
				...
				Q_strcat( neededpaks, len, ".pak" );

				// Local name
				Q_strcat( neededpaks, len, "@");
				// Do we have one with the same name?
				if ( FS_SV_FileExists( va( "%s.pak", fs_serverReferencedPakNames[i] ) ) )
				{
					char st[MAX_ZPATH];
					// We already have one called this, we need to download it to another name
					// Make something up with the checksum in it
					Com_sprintf( st, sizeof( st ), "%s.%08x.pak", fs_serverReferencedPakNames[i], fs_serverReferencedPaks[i] );
...
Q3 media pak0.pak, you'll want to remove this by defining
				...
				&& pakBasename[6] >= '1' && pakBasename[6] <= '1' + NUM_SP_PAKS - 1)
		{
				...
				curpack->checksum != sp_sppak_checksums[pakBasename[6]-'1'] )
			{
				if(pakBasename[6] == '1')
				{
					Com_Printf("\n\n"
						"**************************************************\n"
						"WARNING: " BASEGAME "/sp_pak1.pak is present but its checksum (%u)\n"
						"is not correct. Please re-copy sp_pak1.pak from your\n"
				...
				else
				{
					Com_Printf("\n\n"
						"**************************************************\n"
						"WARNING: " BASEGAME "/sp_pak%d.pak is present but its checksum (%u)\n"
		...
		else
		{
			...
			for(index = 0; index < ARRAY_LEN( en_sppak_checksums ); index++)
			{
					...
					curpack->checksum == sp_sppak_checksums[index] )
				{
					Com_Printf("\n\n"
						"**************************************************\n"
						"WARNING: %s is renamed pak file %s%csp_pak%d.pak\n"
	...
	if(!com_standalone->integer && (foundPak & 0xf) != 0xf)
	{
		...
		if((foundPak & 0xf) != 0xf)
		{
			for( i = 0; i < NUM_SP_PAKS; i++ ) {
				if ( !( foundPak & ( 1 << i ) ) ) {
					Q_strcat( missingPaks, sizeof( missingPaks ), va( "sp_pak%d.pak ", i + 1 ) );
				}
			}
...
different com_basegame was set.
Note: If you're building a game that doesn't depend on the
RTCW media pak0.pak, you'll want to remove this by defining
STANDALONE in q_shared.h
===================
...
static void FS_CheckPak0( void )
{
				...
				&& pakBasename[3] >= '0' && pakBasename[3] <= '0' + NUM_ID_PAKS - 1)
		{
			if( curpack->checksum != pak_checksums[pakBasename[3]-'0'] )
			{
				if(pakBasename[3] == '0')
				{
					Com_Printf("\n\n"
						"**************************************************\n"
						"WARNING: " BASEGAME "/pak0.pak is present but its checksum (%u)\n"
						"is not correct. Please re-copy pak0.pak from your\n"
						"legitimate RTCW CDROM.\n"
						"**************************************************\n\n\n",
						curpack->checksum );
				...
				else
				{
					Com_Printf("\n\n"
							"**************************************************\n"
							"WARNING: " BASEGAME "/pak%d.pak is present but its checksum (%u)\n"
							"is not correct. Please re-install the point release\n"
							"**************************************************\n\n\n",
		...
		else
		{
			...
			for(index = 0; index < ARRAY_LEN(pak_checksums); index++)
			{
				if(curpack->checksum == pak_checksums[index])
				{
					Com_Printf("\n\n"
						"**************************************************\n"
						"WARNING: %s is renamed pak file %s%cpak%d.pak\n"
						"Running in standalone mode won't work\n"
						"Please rename, or remove this file\n"
						"**************************************************\n\n\n",
	...
	if(!com_standalone->integer)
	{
		if(!(foundPak & 0x01))
		{
			if(founddemo)
			{
				Com_Printf( "\n\n"
					"**************************************************\n"
					"WARNING: It looks like you're using pak0.pak\n"
	...
	if(!com_standalone->integer && (foundPak & 0x01) != 0x01)
	{
		...
		{
			Q_strcat(errorText, sizeof(errorText),
				"\n\n\"pak0.pak\" is missing. Please copy it\n"
...
//#endif
	...
	// graphics screen when the font fails to load
	if ( FS_ReadFile( "default.cfg", NULL ) <= 0 ) {
		// this might happen when connecting to a pure server not using BASEGAME/pak0.pak
