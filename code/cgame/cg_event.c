...
//----(SA)	end
	...
	case EV_FALL_DMG_10:
		...
		// use normal pain sound trap_S_StartSound( NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*pain100_1.opus" ) );
		if ( clientNum == cg.predictedPlayerState.clientNum ) {
			// smooth landing z changes
			cg.landChange = -16;
			cg.landTime = cg.time;
	...
	case EV_FALL_DMG_25:
		...
		trap_S_StartSound( NULL, es->number, CHAN_AUTO, CG_CustomSound( es->number, "*fall2.opus" ) );
	...
	case EV_EXERT1:
		DEBUGNAME( "EV_EXERT1" );
		trap_S_StartSound( NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*exert1.opus" ) );
		break;
	case EV_EXERT2:
		DEBUGNAME( "EV_EXERT2" );
		trap_S_StartSound( NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*exert2.opus" ) );
		break;
	case EV_EXERT3:
		DEBUGNAME( "EV_EXERT3" );
		trap_S_StartSound( NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*exert3.opus" ) );
		break;
		...
		DEBUGNAME( "EV_ITEM_PICKUP" );
		{
			...
			if ( event == EV_ITEM_PICKUP ) { // not quiet
				...
				if ( item->giType == IT_POWERUP || item->giType == IT_TEAM ) {
					trap_S_StartSound( NULL, es->number, CHAN_AUTO, trap_S_RegisterSound( "sound/items/n_health.opus" ) );
				} else {
...
//----(SA)	end
	...
	case EV_GENERAL_SOUND:
		...
		if ( !strstr( s, ".opus" ) ) {
			...
			//Q_strcat( tempStr, sizeof( tempStr ), ".opus" );
			//s = tempStr;
		}
		// done.
	...
	case EV_DEATH3:
		...
		if (CG_WaterLevel(cent) == 3) {
			trap_S_StartSound(NULL, es->number, CHAN_VOICE, CG_CustomSound(es->number, "sound/player/gurp1.opus"));
		} else {
...
//				VectorScale( dir, 16, sprVel );
	...
	case EV_POWERUP_BATTLESUIT:
		...
		trap_S_StartSound( NULL, es->number, CHAN_ITEM, trap_S_RegisterSound( "sound/items/protect3.opus" ) );
		break;
	case EV_POWERUP_REGEN:
		...
		trap_S_StartSound( NULL, es->number, CHAN_ITEM, trap_S_RegisterSound( "sound/items/regen.opus" ) );
		break;
