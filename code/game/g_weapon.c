...
// jpw
		...
		if ( traceEnt->health >= 255 ) {
			...
			if ( traceEnt->mg42BaseEnt > 0 ) {
				g_entities[ traceEnt->mg42BaseEnt ].health = 500;
...
// JPW NERVE
	if ( grenType == WP_GRENADE_SMOKE ) {
		...
		te->s.eventParm = G_SoundIndex( "sound/scenaric/forest/me109_flight.opus" );
...
void weapon_zombiespit( gentity_t *ent ) {
	...
	if ( m ) {
		G_AddEvent( ent, EV_GENERAL_SOUND, G_SoundIndex( "sound/Loogie/spit.opus" ) );
	}
#endif
}
