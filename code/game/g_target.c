...
void SP_target_speaker( gentity_t *ent ) {
	...
	//if (!strstr( s, ".opus" )) {
	//	Com_sprintf (buffer, sizeof(buffer), "%s.opus", s );
	//} else {
...
By default this sound is "sound/movers/doors/default_door_locked.opus"
...
void SP_target_relay( gentity_t *self ) {
	...
	if ( !( self->spawnflags & 32 ) ) {  // !NO_LOCKED_NOISE
		if ( G_SpawnString( "lockednoise", "0", &sound ) ) {
			...
			self->soundPos1 = G_SoundIndex( "sound/movers/doors/default_door_locked.opus" );
		}
	}
