...
void Team_ReturnFlagSound( gentity_t *ent, int team ) {
	...
	te->s.eventParm = G_SoundIndex( team == TEAM_RED ?
									"sound/teamplay/flagret_red.opus" :
									"sound/teamplay/flagret_blu.opus" );
...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	te->s.eventParm = G_SoundIndex( our_flag == PW_REDFLAG ?
									"sound/teamplay/flagcap_red.opus" :
									"sound/teamplay/flagcap_blu.opus" );
	te->r.svFlags |= SVF_BROADCAST;
...
void SP_team_WOLF_checkpoint( gentity_t *ent ) {
	...
	G_SpawnString( "noise", "sound/movers/doors/door6_open.opus", &capture_sound );
